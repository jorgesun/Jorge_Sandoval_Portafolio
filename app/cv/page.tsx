        ...
        <>
          ...
          {mode === "light" && (
                  <div style={{ background: "#EFF6FF", borderBottom: "1px solid #BFDBFE", padding: "8px 20px", fontSize: "12px", color: "#1E40AF", display: "flex", alignItems: "center", gap: "8px" }}>
                            <span>Tip: En el dialogo de impresion activa Background graphics, selecciona Save as PDF y desactiva headers/footers.</span>span>
                  </div>div>
              )}
              ...
              <div id="cv-content" style={{ background: t.bg, fontFamily: "'Source Sans 3', sans-serif", color: t.textSecondary, transition: "all 0.4s ease" }}>
                      ...
                      <div style={{ background: t.headerBg, borderBottom: "1px solid " + t.pitchBorder, padding: "38px 48px 32px", position: "relative", overflow: "hidden" }}>
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "20px", position: "relative", zIndex: 1 }}>
                                            <div>
                                                          <h1 style={{ margin: 0, fontSize: "40px", fontWeight: "800", fontFamily: "'Playfair Display', Georgia, serif", color: "#FFFFFF", letterSpacing: "1px", lineHeight: 1.1 }}>Jorge Sandoval</h1>h1>
                                            </div>div>
                                            <div style={{ display: "flex", gap: "20px", flexWrap: "wrap", fontSize: "12px", color: t.textSubtitle }}>
                                              {([{"text": "Tijuana, B.C."}, {"text": "664-520-4587"}, {"text": "jorgesun@gmail.com"}, {"text": "linkedin.com/in/jorgesandoval"}]).map((item, i) => (
                          <span key={i} style={{ display: "flex", alignItems: "center", gap: "5px" }}>{item.text}</span>span>
                        ))}
                                            </div>div>
                                </div>div>
                      </div>div>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: "0" }}>
                                ...
                                <div style={{ padding: "32px 36px 32px 48px", borderRight: "1px solid " + t.colDivider }}>
                                            <SectionTitle icon="+" theme={t}>Experience</SectionTitle>SectionTitle>
                          ...
                          <div key={i}
                                              style={{ marginBottom: "20px", position: "relative", padding: "10px 14px", borderRadius: "6px", background: hoveredExp === i ? t.cardHover : "transparent", transition: "background 0.2s ease" }}
                                            ...
                                            <h3 style={{ margin: "0 0 2px", fontSize: "13px", fontWeight: "700", color: t.expTitle, fontFamily: "'Playfair Display', Georgia, serif" }}>{exp.title}</h3>h3>
                                            <p style={{ margin: "0 0 6px", fontSize: "12px", color: t.accent, fontWeight: "600" }}>
                                              {exp.company} {exp.location}
                                                                <span style={{ float: "right", color: t.textMuted, fontWeight: "400", fontSize: "11px" }}>{exp.period}</span>span>
                                            </p>p>
                                ...
                                <div style={{ padding: "32px 28px" }}>
                                            <SectionTitle icon="*" theme={t}>Skills</SectionTitle>SectionTitle>
                                            <div style={{ marginBottom: "22px", display: "flex", flexWrap: "wrap" }}>
                                              {skills.map((s, i) => (
                          <span key={i} style={{ display: "inline-block", padding: "3px 8px", borderRadius: "4px", fontSize: "10px", fontWeight: "600", letterSpacing: "0.4px", marginRight: "4px", marginBottom: "4px", backgroundColor: t.badgeBg, color: t.badgeText, border: "1px solid " + t.badgeBorder }}>{s}</span>span>
                        ))}
                                            </div>div>
                                
                                            <SectionTitle icon="#" theme={t}>Tech Stack</SectionTitle>SectionTitle>
                                            <div style={{ marginBottom: "22px" }}>
                                              {techStackCategories.map((cat, i) => {
                          const color = mode === "dark" ? cat.darkColor : cat.lightColor;
                          return (
                                              <div key={i} style={{ marginBottom: "12px" }}>
                                                                  <p style={{ margin: "0 0 5px", fontSize: "10px", fontWeight: "700", letterSpacing: "1px", textTransform: "uppercase", color }}>{cat.category}</p>p>
                                                                  <div style={{ display: "flex", flexWrap: "wrap" }}>
                                                                    {cat.tools.map((tool, j) => (
                                                                        <span key={j} style={{ display: "inline-block", padding: "3px 8px", borderRadius: "4px", fontSize: "10px", fontWeight: "600", letterSpacing: "0.4px", marginRight: "4px", marginBottom: "4px", backgroundColor: color + "18", color: color + "CC", border: "1px solid " + color + "30" }}>{tool}</span>span>
                                                                      ))}
                                                                  </div>div>
                                              </div>div>
                                            );
                                            ...
                                            <SectionTitle icon="@" theme={t}>Education</SectionTitle>SectionTitle>
                                            <div style={{ marginBottom: "22px" }}>
                          ...
                          <div key={i} style={{ marginBottom: "10px" }}>
                                            <p style={{ margin: 0, fontSize: "12px", fontWeight: "600", color: t.eduTitle }}>{edu.school}</p>p>
                                            <p style={{ margin: "2px 0 0", fontSize: "11px", color: t.eduSub }}>{edu.degree} {edu.year}</p>p>
                          </div>div>
                        ))}
                                            </div>div>
                                
                                            <SectionTitle icon="!" theme={t}>Certificates</SectionTitle>SectionTitle>
                                            <div style={{ marginBottom: "22px" }}>
                          ...
                          <p key={i} style={{ margin: "0 0 5px", fontSize: "11px", color: t.textBody, paddingLeft: "12px", position: "relative" }}>
                            ...
                            {c}
                                            ...
                                            <SectionTitle icon="&" theme={t}>Awards</SectionTitle>SectionTitle>
                                            <div>
                                              {awards.map((a, i) => (
                          <div key={i} style={{ marginBottom: "9px", padding: "8px 11px", borderRadius: "5px", background: t.awardBg, border: "1px solid " + t.awardBorder }}>
                                            <p style={{ margin: 0, fontSize: "11px", fontWeight: "600", color: t.awardText }}>{a.name}</p>p>
                                            <p style={{ margin: "2px 0 0", fontSize: "10px", color: t.textMuted }}>{a.org} {a.year}</p>p>
                          </div>div>
                        ))}
                                            </div>div>
                                </div>div>
